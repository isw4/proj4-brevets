...
# cutoff distance brackets for each speed bracket (km)
CUTOFF = [0, 200, 400, 600, 1000]
...
def open_time(control_dist_km, brevet_dist_km, brevet_start_time):
    ...
    try:
        check_control_distance(control_dist_km, brevet_dist_km)
    except ValueError:
        raise
...
def check_control_distance(control_dist_km, brevet_dist_km):
    ...
    if control_dist_km > 1.1*brevet_dist_km:
        raise ValueError("The provided control distance is greater than 110% of provided Brevet distance")
    if control_dist_km < 0:
        raise ValueError("The provided control distance is less than 0")
    return False
